			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					if(param != null && param != undefined){
						//TODO 自定义新规则
						var type = param.type != undefined ? param.type : "get";
						var dataType = param.dataType != undefined ? param.dataType : "json";
						var c_data  = param.data != undefined ? param.data : {};
						var requestData = {};
						//post请求
						if(param.data != undefined && type != "get"){
							for(var index in c_data){
								var id = "#" + c_data[index];
								if($(id).val() == undefined){
									continue;
								}
								requestData[index] = $(id).val();
							}
						}
						else{ //get请求
						...
						var coachId_=$("#coachId").val();
						var subjectId_=$("#subjectId").val();
						...
						if(data != null && $.isArray(data) ){
							...
							$.post(url,{keyword:keyword_,coachId:coachId_,subjectId:subjectId_},function(result){
								makeContAndShow(result.data)
							},"json")
						}
					}
