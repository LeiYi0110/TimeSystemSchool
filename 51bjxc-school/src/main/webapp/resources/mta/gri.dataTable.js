 ...
 *           config['allFields'] = {
                'name':
                {
                    ...
                    'number':false,
                    'colAlign':'left',
                    'thAlign' :'center',//新增头部列对齐
                    'thIndent':'40px'//新增头部偏移
                }
            }
 */
;
        ...
        createTable:function (options) {
            ...
            if (ifDivNode) {
                ...
                if (document.all) {
                    ...
                    for (var key in allFields) {
                        ...
                        if (options.thIndent) {
                            _th.style.textIndent = options.thIndent;
                        }
                    ...
                    for (var key in allFields) {
                        ...
                        if (key != 'gri_checkAll') {
                            ...
                            if (allFields[key].thText) {
                                thStr += ('<th name=' + key + ' ');
                                thStr += "  style=";
                                if(allFields[key].thAlign){
                                    thStr += "'text-align:" + allFields[key].thAlign + ";'";
                                }else if (options.thAlign) {
                                    thStr += "'text-align:" + options.thAlign + ";'";
                                }
                                if(allFields[key].thIndent){
                                    thStr += "'text-indent:" + allFields[key].thIndent + ";'";
                                }else if (options.thIndent) {
                                    thStr += "'text-indent:" + options.thIndent + ";'";
                                }
            ...
            for (var i = 0; i < this.page.data.length; i++) {
                ...
                for (var name in allFields) {
                	...
                	columnIndex++;
                    //初始化formater和行对应的对象
                    var _field = allFields[name];
                    var _render = _field.render;
                    var verticalAlign = _field.verticalAlign;
                    verticalAlign = verticalAlign ? verticalAlign : 'inherit';
                    
                    //使用数据列定义中的数据格式
                    var formater = _field.formater;
                    if(!formater){
                    	formater = this.Formater.colNormalFormater;
                    }
                    ...
                    if(_render){
                    	 ...
                    	 tr += '<td style="vertical-align: ' + verticalAlign +';';
                    	 
                    	 tr += '">';
                    	 tr += _render(fieldData,_field,row,i,columnIndex);
                    	 tr += '</td>';
                    	//跳出本列的渲染
                    	continue;
                    }
                    ...
                    //如果没有配置阀值列
                        ...
                        _data.verticalAlign = verticalAlign;
...
//                   上面循环在ie死循环 - 于 2016.12.12 修改一下方法
                    var arrTr = trRow.split('</td>');
                    for( var j=0; j<arrTr.length-1; j++ ){
                    	arrTr[j] += '</td>';
                    	var tdNode = document.createElement('TD');
                    	tdNode.innerHTML = decodeURIComponent(arrTr[j]);
                    	trNode.appendChild(tdNode);
                    }
...
//				ie下选择一行的id 于 2016.12.12 修改                
                var myTbody = _table.getElementsByTagName('tbody')[0];
                var myTrs = myTbody.getElementsByTagName('tr');
	            for (var i = 0; i < myTrs.length; i++) {
	            	for(var j=0; j<myTrs[i].cells.length; j++){ //取得第几行下面的td个数，再次循环遍历该行下面的td元素
	                   var cell = myTrs[i].cells[j];//获取某行下面的某个td元素
	                   cell.tag = i;
	                   cell.onclick=function(event){
	                	   var _tr = myTbody.getElementsByTagName('tr')[this.tag];
	                	   var _td = _tr.getElementsByTagName('td');
	                	   if(!_td) return;
	                	   var _idDom = _td[0].getElementsByTagName('span')[0];
	                	   if(!_idDom) return;
	                	   var _id = _idDom.getAttribute('data-id');
	                	   sessionStorage.setItem("tabId", _id);
	                   }; 
	                }
	            }
                ...
                //隔行换色
                var _trs = container.getElementsByTagName("TABLE")[0].getElementsByTagName('tbody')[0].getElementsByTagName('tr');
                for (var i = 0; i < _trs.length; i++) {
                	
                	for(var j=0;j<_trs[i].cells.length;j++)//取得第几行下面的td个数，再次循环遍历该行下面的td元素
                    {
                       var cell = _trs[i].cells[j];//获取某行下面的某个td元素
                       cell.tag = i;
                       cell.onclick=function(event){
                    	   var _tr = document.getElementsByTagName('tr')[this.tag + 1];
                    	   var _td = _tr.getElementsByTagName('td');
                    	   if(!_td) return;
                    	   var _idDom = _td[0].getElementsByTagName('span')[0];
                    	   if(!_idDom) return;
...
//                    }
            ...
            for (var i = 0; i < _tableTrs.length; i++) {
                ...
                _tr.onclick = function () {
                	for(var i = 0; i < _tableTrs.length; i++){
                		_tableTrs[i].setAttribute('class','');
        ...
        createPage:function () {
            ...
            if (this.page.index < this.page.count) {
                ...
                if (this.page.theme == 'lite') {
                    ...
                    spanTotal.innerHTML = ("共<em>" + this.rowCount + "</em>条，每页");
        ...
        createJsonTable:function (_this) {
           ...
           // loadingDiv.innerHTML = "<img src='http://gri.oa.com/portal/resource/scripts/img/loading.gif' alt='加载中...' />";
        	...
        	var paramStr = 'pageIndex=' + _this.page.index + '&pageSize=' + _this.page.size;
        	if(_this.page.orderField){
        		paramStr +='&orderField=' + _this.page.orderField;
        	}
        	if(_this.page.orderType){
        		paramStr +='&orderType=' + _this.page.orderType;
        	}
        	paramStr +='&t=' + Math.random();
        	
        	var params = _this.page.params;
            ...
            _this.ajaxGet(sendUrl,params, function (result, _this) {
                ...
                if (resultObj.data) {
                    ...
                    _this.options.page.rowCount = resultObj.rowCount;
                } else {
                	_this.options.data = [];
         ...
         */
        ajaxGet:function (url,params, fn, object) {
        	GRI.Util.loading.show(object.container||object.options.tableId);
        	$.getJSON(url,params,function(data){
        		if(data && data.code==200){
        			MTA.Data.tableData = data;
        			fn(data, object);
        		}
        		GRI.Util.loading.clear(object.container||object.options.tableId);
        	});
        	/**
        ...
        Formater:{
            ...
            colNormalFormater:"<td style='vertical-align:{$verticalAlign}'>{$data}</td>",
     ...
     * @param {object} param 请求参灵敏.
     * @param {function} fn 获取结果之后的回调函数.
     * @param {object} obj 回调函数可能要用到的对象.
     */
    global.Gri.ajaxGet = function (url,params, fn, config) {
    	var paramStr = 'pageIndex=0&pageSize=' + config.page.size;
    	if(config.page.orderField){
    		paramStr +='&orderField=' + config.page.orderField;
    	}
    	if(config.page.orderType){
    		paramStr +='&orderType=' + config.page.orderType;
    	}
    	paramStr +='&t=' + Math.random();
        config.page.params = params;
        var sendUrl = config.page.url;
        if (config.page.url.indexOf('?') > 0) {
            sendUrl = sendUrl + '&' + paramStr;
        } else {
            sendUrl = sendUrl + '?' + paramStr;
        }
        new DataTable().ajaxGet(sendUrl,params, fn, config);
    };
