...
<body>
	...
	<!-- <a class="select-btn" data-toggle="modal" onclick="showEditorModel()"><span class="update"></span>处理意见</a> -->
		...
		<sec:authorize access="hasRole('ROLE_SCHOOL_SERVICE')">
			<sec:authorize access="hasRole('ROLE_UserOperation_184')">
				<sec:authorize access="hasRole('ROLE_TIME_SYSTEM')">
					<a class="select-btn" onClick="upRecord()"><span class="spare"></span>备案</a>
				</sec:authorize>
			</sec:authorize>
			<sec:authorize access="hasRole('ROLE_UserOperation_185')">
				<a class="select-btn" onclick="exportFile();" ><span class="Export"></span>导出</a>
			</sec:authorize>
			<sec:authorize access="hasRole('ROLE_UserOperation_186')">
				<a class="select-btn" data-toggle="modal" onclick="addModal();"><span class="add"></span>录入投诉</a>
			</sec:authorize>
		</sec:authorize>
		</div>
			...
			<div class="modal-content">
				<div class="left">
					...
					<div class="row1">
						...
						<div class="form-controll">
							<label>学员身份证号:</label>
							<input type="text" name="identifierNo" datatype="*" errormsg="身份证号" nullmsg="身份证号不能为空" sucmsg=" ">
						</div>
					</div>
					...
					<div class="row1">
						<div class="form-controll">
						<label>教练编号:</label>
							<input type="text" name="coachnum" datatype="*" nullmsg="教练编号不能为空" sucmsg=" ">			
						</div>
						<div class="form-controll">
							<label>投诉教练:</label>
							<input type="text" name="coachName" datatype="*" nullmsg="投诉教练不能为空" sucmsg=" ">
						</div>
					</div>
					<div class="row1">
	...
	<script type="text/javascript">
			...
			config['allFields'] = {
				...
				'status' :{
					...
					'render' : function(value, cellmeta, record, rowIndex,
						...
						var td = [];
						var text = "";
						if(Number(record.reserveCount) == 0){
							//text ='无效投诉无需处理/';
						}
						if(Number(value) == 0){
							text +='未处理';
		...
		*/
					...
					if(result.code==200){
						...
						var dialog = new GRI.Dialog({
							...
							var params = {
									'stunum' :data.stunum,
									'type' : data.type,
									'objenum' : data.coachnum,//'2383114336134796',
									'cdate' : cdate,
		...
		function exportFile(){
			window.location.href="${ctx}/complaint/exportComplaint?type="+1;
		}
		function addModal(){
			//重置表单
			var tool = new toolCtrl();
		...
		function saveInfo() {
			...
			var params = {
				'type' : 1,
				'content' : content,
				'coachName' : $('#addComplaintsDriving input[name="coachName"]').val(),
				'coachnum' : $('#addComplaintsDriving input[name="coachnum"]').val(),
