...
<body>
	...
	<script type="text/javascript">
		var current_action = "${ctx}/view/mg/securityEditor"; 
		var id = '${param.id}';
	...
	function save(){
		...
		$.post('http://192.168.1.6:8000/upload/file/url',fileParam,function(urlResult) {
			...
			//发送数据到后端保存
			$.post('http://192.168.1.6:8000/country/securityguard',param,function(result) {
					...
					$.post('${ctx}/security/saveOrUpdate', params, function(result) {
						if (result.code == 200) {
							var dialog = new GRI.Dialog({
								...
								if(!id || id != ''){
									window.location.href = "${ctx}/view/securityMember";
								}
							});
						} else {
							//打开错误提示框
